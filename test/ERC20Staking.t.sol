...
contract ETHStakingTest is Test {
    ...
    function setUp() public {
        ...
        stakingDataContract.setStakeToken(address(stakeTokenContract));
    }
    ...
    function testRedeemRewards() public {
        ...
        (success, ) = address(stakingDataContract).call(abi.encodeWithSignature("stake(address,uint256)", address(customTokenContract), 10**19));
        assertTrue(success);
        ...
        (success, ) = address(stakingDataContract).call(abi.encodeWithSignature("redeemRewards()"));
        assertTrue(success);

        uint256 timeTillRedeem = stakingDataContract.rewardClaims(user);
        ...
        uint256 userTokenBalance = stakeTokenContract.balanceOf(user);
